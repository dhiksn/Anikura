...
export default async function OngoingPage(props: {
        ...
        <div className="flex-1 min-w-0">
          {data?.length > 0 ? (
              ...
              {pagination && (pagination.hasNextPage || pagination.hasPrevPage) && (
                <div className="flex items-center justify-center gap-4 pt-8 border-t border-border/40">
                  {pagination.hasPrevPage ? (
                    <Link href={`/ongoing?page=${pagination.prevPage}`} className={buttonVariants({ variant: "outline" })}>
                  ...
                  {pagination.hasNextPage ? (
                    <Link href={`/ongoing?page=${pagination.nextPage}`} className={buttonVariants({ variant: "outline" })}>
            ...
            <div className="flex flex-col items-center justify-center text-center h-64 text-muted-foreground">
              <p>Tidak ada anime yang sedang tayang saat ini.</p>
            </div>
          )}
        </div>
