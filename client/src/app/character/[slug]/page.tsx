...
import { getAnimeByCharacter } from "@/lib/api";
...
export default async function CharacterDetailPage(props: {
  ...
  try {
    results = await getAnimeByCharacter(params.slug, page);
  } catch (err: any) {
    error = err.response?.data?.error?.message || "Gagal memuat anime untuk karakter ini.";
  }

  if (error || !results?.character) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        <h1 className="text-3xl font-bold tracking-tight mb-2">Tidak Ditemukan</h1>
        <p className="text-muted-foreground mb-8">{error || "Karakter tidak ditemukan."}</p>
        <Link href="/character" className={buttonVariants({ variant: "default" })}>Kembali ke Karakter</Link>
      </div>
    );
  }

  const { character, data, pagination } = results;
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl py-12 flex flex-col gap-12">
      <div className="flex flex-col gap-3 border-b border-border/40 pb-8">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Link href="/character" className="hover:text-foreground transition-colors">Tipe Karakter</Link>
          <span>/</span>
          <span className="text-foreground">{character.name}</span>
        </div>
        <h1 className="text-4xl md:text-5xl font-bold tracking-tighter">{character.name}</h1>
        <p className="text-muted-foreground text-lg max-w-[60ch]">
          Koleksi anime dengan tipe karakter {character.name.toLowerCase()}.
        </p>
      </div>
      ...
      {data?.length > 0 ? (
          ...
          {pagination && (pagination.hasNextPage || pagination.hasPrevPage) && (
            <div className="flex items-center justify-center gap-4 pt-8 border-t border-border/40">
              {pagination.hasPrevPage ? (
                <Link href={`/character/${params.slug}?page=${pagination.prevPage}`} className={buttonVariants({ variant: "outline" })}>
              ...
              {pagination.hasNextPage ? (
                <Link href={`/character/${params.slug}?page=${pagination.nextPage}`} className={buttonVariants({ variant: "outline" })}>
        ...
        <div className="flex flex-col items-center justify-center text-center h-64 text-muted-foreground">
          <p>Tidak ada anime untuk karakter ini.</p>
        </div>
      )}
    </div>
  );
