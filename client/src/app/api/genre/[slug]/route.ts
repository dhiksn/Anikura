...
const { scrapeGenreAnime } = require('@/lib/api-server/services/genre.service');
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const data = await scrapeGenreAnime(slugVal.slug, pageVal.page);
    return ok(data.animeList, { genre: data.genre, total: data.total, pagination: data.pagination });
