...
const { scrapeCharacterAnime } = require('@/lib/api-server/services/character.service');
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const data = await scrapeCharacterAnime(slugVal.slug, pageVal.page);
    return ok(data.animeList, { character: data.character, total: data.total, pagination: data.pagination });
