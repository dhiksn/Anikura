...
import { getAnimeByStudio } from "@/lib/api";
...
export default async function StudioPage(props: {
  ...
  try {
    results = await getAnimeByStudio(params.slug, page);
  } catch (err: any) {
    error = err.response?.data?.error?.message || "Gagal memuat anime untuk studio ini.";
  }

  if (error || !results?.studio) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        <h1 className="text-3xl font-bold tracking-tight mb-2">Tidak Ditemukan</h1>
        <p className="text-muted-foreground mb-8">{error || "Studio tidak ditemukan."}</p>
        <Link href="/" className={buttonVariants({ variant: "default" })}>Kembali ke Beranda</Link>
      </div>
    );
  }

  const { studio, data, pagination } = results;
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl py-12 flex flex-col gap-12">
      <div className="flex flex-col gap-3 border-b border-border/40 pb-8">
        ...
        <h1 className="text-4xl md:text-5xl font-bold tracking-tighter">Anime Garapan {studio.name}</h1>
        <p className="text-muted-foreground text-lg max-w-[60ch]">
          Koleksi anime yang diproduksi oleh {studio.name}.
        </p>
      </div>
      ...
      {data?.length > 0 ? (
          ...
          {pagination && (pagination.hasNextPage || pagination.hasPrevPage) && (
            <div className="flex items-center justify-center gap-4 pt-8 border-t border-border/40">
              {pagination.hasPrevPage ? (
                <Link href={`/studio/${params.slug}?page=${pagination.prevPage}`} className={buttonVariants({ variant: "outline" })}>
              ...
              {pagination.hasNextPage ? (
                <Link href={`/studio/${params.slug}?page=${pagination.nextPage}`} className={buttonVariants({ variant: "outline" })}>
        ...
        <div className="flex flex-col items-center justify-center text-center h-64 text-muted-foreground">
          <p>Tidak ada anime untuk studio ini.</p>
        </div>
      )}
    </div>
  );
