...
/**
 * Parse pagination — format /studio/{slug}/page/{N}/
 */
...
/**
 * Scrape daftar anime berdasarkan studio.
 * URL format: /studio/{slug}/ atau /studio/{slug}/page/{N}/
 *
 * @param {string} slug  - slug studio, mis. "netflix", "mappa"
 * @param {number} [page=1]
 * @returns {Promise<object>}
 */
async function scrapeStudio(slug, page = 1) {
  const studioUrl = `${BASE_URL}/studio/${slug}/`;
  const url       = page > 1 ? `${studioUrl}page/${page}/` : studioUrl;
  const html      = await fetchHtml(url, { useCache: true });
  const $         = load(html);

  const pageTitle = cleanText($('title').text()).toLowerCase();
  if (pageTitle.includes('404') || pageTitle.includes('not found')) {
    const err = new Error(`Studio "${slug}" tidak ditemukan`);
    err.statusCode = 404;
    err.code = 'NOT_FOUND';
    throw err;
  }
  ...
  return {
    studio: { name: studioName, slug, url: studioUrl },
    animeList,
