...
const { scrapeOngoing }  = require('../services/ongoing.service');
const { validatePage }   = require('../utils/validator');

/**
 * GET /api/ongoing?page=1
 * Mengambil daftar anime yang sedang tayang dengan pagination.
 */
async function getOngoing(req, res, next) {
  try {
    ...
    const result = await scrapeOngoing(pageValidation.page);
