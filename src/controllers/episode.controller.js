'use strict';

const { scrapeEpisode }  = require('../services/episode.service');
const { validateTargetUrl } = require('../utils/validator');

/**
 * GET /api/episode?url=https://animasu.love/nonton-...
 * Mengambil informasi halaman episode beserta daftar server streaming.
 */
async function getEpisode(req, res, next) {
  try {
    const urlValidation = validateTargetUrl(req.query.url);
    if (!urlValidation.valid) {
      return res.status(400).json({
        success: false,
        error: {
          code:    'INVALID_PARAMETER',
          message: urlValidation.message,
        },
      });
    }

    const targetUrl = req.query.url.trim();
    const data      = await scrapeEpisode(targetUrl);

    if (!data.title) {
      return res.status(404).json({
        success: false,
        error: {
          code:    'NOT_FOUND',
          message: 'Data episode tidak ditemukan pada URL tersebut',
        },
      });
    }

    return res.status(200).json({
      success: true,
      source:  'Animasu',
      url:     targetUrl,
      data,
    });
  } catch (err) {
    next(err);
  }
}

module.exports = { getEpisode };
